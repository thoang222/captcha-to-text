...
#             saved_model_dir = onnx_model_path.replace(".onnx", "_saved_model")
...
#                 onnx.save(onnx_model, onnx_model_path)
#         except Exception as e:
#             print(f"Error during metadata inclusion: {e}")  
...
#     def on_train_end(self, logs=None):
#         """ Converts the model to onnx format after training is finished. """
#         onnx_model_path = self.saved_model_path.replace(".keras", ".onnx")
#         self.model2onnx(self.model, onnx_model_path)
#         self.include_metadata(onnx_model_path, self.metadata)
